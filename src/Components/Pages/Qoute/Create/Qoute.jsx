...
export default function SimpleContainer() {
    ...
    React.useEffect(() => {
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/qoute`, { headers: { 'Authentication': "Bearer " + localStorage.getItem('token') } })
    ...
    const onSubmit = async (data) => {
        ...
        if (item.length > 0 && submit) {
            await axios.post(`${process.env.REACT_APP_BACKEND_URL}/qoute`, finalResult,
                {
                    headers: {
            ...
            <Container maxWidth="1000px">
                <Box className="invoice-main-container">
                    <Box className='d-flex'>
                        ...
                        <label htmlFor="invoice-form" className='form-label'>CREATE - Qoute</label>
                    </Box>
                            ...
                            <Box className='d-flex'>
                                <Box>
                                    <Box className="form-input-colum">
                                        <label htmlFor="invoice-number">Qoute Number</label>
                                        <input className='form-input-field' type="text" name="invoice-number" value={invoiceNumber} disabled {...register("qouteNumber")} />
                                        <label htmlFor="">Qoute Date</label>
                                        <input className='form-input-field' type="date" id='date' value={date.toLocaleDateString()} {...register("quoteDate")} />
